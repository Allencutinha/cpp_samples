...
void detectCannyEdges(uchar* inBuff, uchar* outBuff, uchar* intBuff, int height, int width)
{
    ...
    // non maximum supression of edges, here we reduce the thickness of the
    // previously obtained edges
    for (int row = 1; row < height - 1; row++) {
        for (int col = 1; col < width - 1; col++) {
            ...
            if (edgeDir[row * width + col] == 0) {
                if (G[row * width + col] > G[(row)*width + (col - 1)] &&
                    G[row * width + col] > G[(row)*width + (col + 1)]) {
                    GTemp[row * width + col] = G[row * width + col];
                }
            }
            if (edgeDir[row * width + col] == 45) {
                if (G[row * width + col] > G[(row + 1) * width + (col - 1)] &&
                    G[row * width + col] > G[(row - 1) * width + (col + 1)]) {
                    GTemp[row * width + col] = G[row * width + col];
                }
            }
            if (edgeDir[row * width + col] == 90) {
                if (G[row * width + col] > G[(row - 1) * width + (col)] &&
                    G[row * width + col] > G[(row + 1) * width + (col)]) {
                    GTemp[row * width + col] = G[row * width + col];
                }
            }
            ...
            intBuff[row * width + col] = GTemp[row * width + col] =
                (uchar)((GTemp[row * width + col] * 255) / maxDiff);
        }
    }
    ...
    for (int row = 1; row < height - 1; row++) {
        for (int col = 1; col < width - 1; col++) {
            ...
            if (GTemp[row * width + col] > upperThreshold) {
                GTemp[row * width + col] = 255;
            } else if (GTemp[row * width + col] < lowerThreshold) {
                GTemp[row * width + col] = 0;
            }
            intBuff[row * width + col] = GTemp[row * width + col];
        }
        ...
        for (int row = 1; row < height - 1; row++) {
            for (int col = 1; col < width - 1; col++) {
                ...
                if (GTemp[row * width + col] != 0) {
                    // make the current pixel an edge pixel if any of its 8
