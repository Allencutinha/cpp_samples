...
void ValueBinaryTree<T>::createCompleteTree(const std::vector<T> &contents) {
  ...
  for (unsigned int i = 1; i < contents.size(); i++) {
    ...
    // Diagnostic code:
    // std::cerr << "Added child node with data: " << actualChildPtr->data <<
    // std::endl;
...
template <typename T> void ValueBinaryTree<T>::preOrder(TreeNode *cur) {
...
template <typename T> void ValueBinaryTree<T>::inOrder(TreeNode *cur) {
  // Only do anything if the pointer is not nullptr.
  if (cur) {
...
template <typename T> void ValueBinaryTree<T>::postOrder(TreeNode *cur) {
  // Only do anything if the pointer is not nullptr.
  if (cur) {
    // First, recurse on the left.
    postOrder(cur->left);
