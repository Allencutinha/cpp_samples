...
public:
    ...
    // whole definition.)
    ~ValueBinaryTree() { destroyWholeTree(); }
    ...
    ValueBinaryTree(const std::vector<T>& contents) : ValueBinaryTree() { createCompleteTree(contents); }
    ...
    // pointers this way.
    TreeNode* unsafe_getRootPtr() { return root_; }
