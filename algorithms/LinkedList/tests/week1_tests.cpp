    ...
    SECTION("Timing insertOrdered")
    {
...

        {
            ...
            if (studentList0 != list0_correct) {
                std::cout << "WARNING: It appears insertOrdered isn't correctly "
                             "implemented yet.\n"
        ...
        }
        {
            ...
            std::cout << "The worst case should be O(n) for insertOrdered,\n so the "
                         "larger case might take nearly 10x longer here."
                      << std::endl;
        }
    }
    ...
    SECTION("Timing merge")
    {
        ...
        }
        {
            ...
            std::cout << "The worst case should be O(n) for merge,\n so the larger "
                         "case might take nearly 10x longer here."
                      << std::endl;
        }
    }
}
...
TEST_CASE("Benchmark: Measuring slowdown for sorting algorithms", "[weight=0][.][bench]")
{
    ...
    SECTION("Timing insertionSort")
    {
...

        {
            ...
            if (testSort != sortedListSmall) {
                std::cout << "WARNING: It appears insertionSort or insertOrdered isn't "
        ...
        }
        {
            ...
            if (sortedList.size())
                std::cout << "Time elapsed: " << dur_ms.count() << "ms" << std::endl;
            std::cout << "The worst case is O(n^2) for insertionSort, so the larger "
                         "sort might take\n nearly 100x longer in this case."
                      << std::endl;
        }
    }
    ...
    SECTION("Timing mergeSortRecursive")
    {
...

        {
            ...
            if (testSort != sortedListSmall) {
                std::cout << "WARNING: It appears mergeSortRecursive or merge isn't "
                             "correctly implemented yet.\n"
                          << "  The running times below may not be meaningful." << std::endl;
            }
        }
        ...
        // https://en.cppreference.com/w/cpp/chrono/duration/duration_cast
        {
            ...
            std::chrono::duration<double, std::milli> dur_ms = stop_time - start_time;
            // Make sure the loop doesn't get optimized away.
            if (sortedList.size() != unsortedList1.size())
                std::cout << "WARNING: List size didn't match!" << std::endl;
            if (sortedList.size())
                std::cout << "Time elapsed: " << dur_ms.count() << "ms" << std::endl;
        }
        {
            ...
            auto stop_time = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::milli> dur_ms = stop_time - start_time;
            if (sortedList.size() != unsortedList2.size())
                std::cout << "WARNING: List size didn't match!" << std::endl;
            if (sortedList.size())
                std::cout << "Time elapsed: " << dur_ms.count() << "ms" << std::endl;
            std::cout << "A correct implementation is O(n log n), so the larger sort "
                         "should take\n about 11x-13x longer in this case."
                      << std::endl;
        }
    }
    ...
    SECTION("Timing mergeSortIterative")
    {
        ...
        std::cout << "mergeSortIterative is being tested on much larger lists than "
                     "the others"
                  << std::endl
                  << " because this implementation exhibits extra overhead for "
                     "small list sizes."
                  << std::endl
                  << " The change in running time becomes more clearly O(n log n) "
                     "with inputs this large."
                  << std::endl;
...

        {
            ...
            if (testSort != sortedListSmall) {
                std::cout << "WARNING: It appears mergeSortIterative or merge isn't "
                             "correctly implemented yet.\n"
                          << "  The running times below may not be meaningful." << std::endl;
            }
        }
        ...
        // https://en.cppreference.com/w/cpp/chrono/duration/duration_cast
        {
            ...
            for (int i = 0; i < NUM_TEST_RUNS; i++) {
                ...
                if (!sortedList.isSorted())
                    std::cout << "WARNING: mergeSortIterative result not sorted." << std::endl;
            }
            auto stop_time = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::milli> dur_ms = stop_time - start_time;
            // Make sure the loop doesn't get optimized away.
        ...
        std::cout << std::endl
                  << "Note: Compiler optimizations or overhead might cause some of "
                     "these to run"
                  << std::endl
                  << " faster or slower than expected, even if your implementation "
                     "is correct."
                  << std::endl
                  << " The algorithms aren't all being tested on the same list "
                     "size, but rather on lists"
                  << std::endl
                  << " large enough to show the asymptotic complexity. (Compare "
                     "the ratio of running times"
                  << std::endl
                  << " for a single algorithm before and after the increase in "
                     "input size.)"
                  << std::endl;
    }
}
...
TEST_CASE("Testing insertOrdered: Insert at front", "[weight=1]")
{
    ...
    SECTION("Checking that values are correct") { REQUIRE(studentResultList == expectedList); }

    SECTION("Checking that the list prev links and tail pointer are being set "
            "correctly")
    {
