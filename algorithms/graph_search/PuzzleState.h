...
public:
    ...
    static bool validateArray(const std::array<int, 9>& data)
    {
        ...
        if (dataCopy != std::array<int, 9>({1, 2, 3, 4, 5, 6, 7, 8, 9})) {
            std::cout << "validateArray warning: data must contain a permutation of numbers 1 through 9"
                      << std::endl;
            return false;
        }
        return true;
    ...
    bool operator!=(const PuzzleState& other) const { return (data != other.data); }
    ...
    static PuzzleState getRandomMove(const PuzzleState& curState, const PuzzleState& prevState)
    {
        ...
        for (auto& neighbor: adjacentStates) {
            if (neighbor != prevState)
                validDestinations.push_back(neighbor);
        }
