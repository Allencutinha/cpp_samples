...
void informalTests()
{
    ...
    if (showWelcome) {
        ...
        std::cout << "Make sure to inspect informalTests() in main.cpp to turn on the available" << std::endl
    ...
    if (doEx3Tests) {
        ...
        std::cout << "Now running puzzleBFS tests. Some tests may be disabled for speed; see" << std::endl
                  << "informalTests() in main.cpp for details. If any tests take a very long time,"
                  << std::endl
                  << "you probably have a bug. You can cancel the running process in the terminal"
                  << std::endl
...
void demoRunner()
{

    {
        ...
        std::cout << "At this point, the set contains 7 and 42. (It only has a single copy of 42.)"
                  << std::endl
                  << std::endl;

        std::cout << "Iterating over std::unordered_set contents. (These may not be in any order.)"
                  << std::endl;
        ...
        std::cout << "And then we can use count() to query if an item is in the set or not, and we"
                  << std::endl
...

    {
        ...
        std::cout << "Our GridGraph class doesn't explicitly store edges since it uses an adjacency"
                  << std::endl
        ...
        std::cout << "Creating IntPairPair AB from {pointA, pointB} and BA from {pointB, pointA}."
                  << std::endl;
...

    {
        ...
        std::cout << "Verifying equality:" << std::endl
                  << std::boolalpha << "  point1 == point2 : " << (point1 == point2) << std::endl
                  << "  point2 == point3 : " << (point2 == point3) << std::endl
                  << "  point3 == point4 : " << (point3 == point4) << std::endl
                  << std::noboolalpha << std::endl;

        std::cout << "However, if you're not explicit enough, the compilation may fail." << std::endl
...
void graphBFS_Tester()
{
    ...
    if (path.empty()) {
        ...
        if (!GridGraph::allowVerboseTextDescription && !forceDescribePath) {
            std::cout << "Omitting verbose text description of path. You can change this in main.cpp"
                      << std::endl
        ...
        while (!working_path.empty()) {
            ...
            if (!graph.hasEdge(p1, p2)) {
                ...
                if (forceDescribePath ||
                    (!GridGraph::allowPlotting && GridGraph::allowVerboseTextDescription)) {
                    std::cout << "[(" << p1.first << "," << p1.second << ") to (" << p2.first << ","
                              << p2.second << ")] ";
                }
            }
            // step forward
