...
TEST_CASE("Testing countEdges:", "[weight=1][ex1]")
{
    ...
    SECTION("Should not count each edge twice by mistake") { REQUIRE(numEdges != 14); }
...
TEST_CASE("Testing graphBFS when the goal is unreachable:", "[weight=1][ex2][timeout=60000]")
{
    ...
    SECTION("Should return empty path when goal unreachable") { REQUIRE(path.empty()); }
}
...
TEST_CASE("Testing graphBFS when start==goal:", "[weight=1][ex2]")
{
    ...
    SECTION("Test configuration sanity check") { REQUIRE(maze_start == maze_goal); }

    SECTION("Should not return empty path when start==goal") { REQUIRE(!path.empty()); }
...
TEST_CASE("Testing puzzleBFS when the goal is unreachable (puzzle can't be solved):",
...
TEST_CASE("Testing puzzleBFS when start==goal:", "[weight=1][ex3]")
{
    ...
    SECTION("Test configuration sanity check") { REQUIRE(puzzle_start == puzzle_goal); }
