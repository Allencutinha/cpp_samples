...
int GridGraph::countEdges() const
{
    ...
    for (auto adjMap_iter = adjacencyMap.begin(); adjMap_iter != adjacencyMap.end(); adjMap_iter++) {
        ...
        for (auto edge_iter = edge_pair_container.begin(); edge_iter != edge_pair_container.end();
             edge_iter++) {
...
std::list<IntPair> graphBFS(const IntPair& start, const IntPair& goal, const GridGraph& graph)
{
    ...
    while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
        ...
        if (curPointWasPreviouslyDequeued) {
            std::cout << "graphBFS ERROR: Dequeued a vertex that had already been dequeued before."
                      << std::endl
                      << " If you're using visitedSet correctly, then no vertex should ever be added"
                      << std::endl
...
std::list<PuzzleState> puzzleBFS(const PuzzleState& start, const PuzzleState& goal)
{
    ...
    while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
        ...
        if (curPointWasPreviouslyDequeued) {
            std::cout << "puzzleBFS ERROR: Dequeued a vertex that had already been dequeued before."
                      << std::endl
                      << " If you're using visitedSet correctly, then no vertex should ever be added"
                      << std::endl
                      << " to the explore qeueue more than once. [Returning an empty path now.]" << std::endl
