...
void wordcountTests()
{
    ...
    std::cout << "Loading strings from \"Through the Looking-Glass\" by Lewis Carroll" << std::endl
              << std::endl;
    ...
    std::cout << "Assuming makeWordCounts is working, here are some tests of lookupWithFallback:"
              << std::endl;
    int bandersnatch_count = lookupWithFallback(wordcount_map, "bandersnatch", 0);
    std::cout << "Looking up \"bandersnatch\" with fallback of 0. Result (expecting 3): "
    ...
    if (wordcount_map_backup == wordcount_map)
        std::cout << "Good, wordcount_map wasn't changed by the lookup." << std::endl;
    else
    ...
    if (wordcount_map.count("cheshire"))
        errorReaction("Found \"cheshire\" in the map. That shouldn't have happened.");
    else
        std::cout << "Good, \"cheshire\" was not found, as expected." << std::endl;
    int cheshire_count = lookupWithFallback(wordcount_map, "cheshire", -1);
    std::cout << "Looking up \"cheshire\" with fallback of -1. Result (expecting -1): " << cheshire_count
              << std::endl;
    if (cheshire_count != -1)
        errorReaction("Should not have found \"cheshire\" in map, and should have reported -1 as fallback.");
    else
        std::cout << "Good!" << std::endl;
    std::cout << "Checking that wordcount_map was not modified by the lookup:" << std::endl;
    if (wordcount_map_backup == wordcount_map)
...
void hashingDemo()
{
...

    {
        ...
        std::cout << "Note:\n You don't need to manually invoke our custom hasher to do this assignment."
                  << std::endl
                  << " Instances of std::unordered_map<IntPair, int> will automatically use it." << std::endl
                  << std::endl;
...
void palindromeTests()
{
    ...
    {
        std::cout << "======= Finding the longest palindrome substring (brute force) =======" << std::endl
                  << std::endl;
    ...
    {
        std::cout << "======= Finding the longest palindrome substring (memoized) =======" << std::endl
                  << std::endl;
        std::cout << "Note: If you haven't finished correctly implementing this function yet," << std::endl
                  ...
                  << std::endl
                  << " That could also happen if you have a bug or infinite recursion." << std::endl
                  << std::endl;
        std::cout << "String: \"" << str << "\"" << std::endl;
        ...
        try {
            pal_result =
                memoizedLongestPalindromeLength(memo, str, 0, str.length() - 1, start_time, max_duration);
        } catch (TooSlowException& e) {
            timed_out = true;
        }
        ...
        if (timed_out) {
            ...
            if (have_intended_result) {
                if (pal_result != intended_pal_result)
                    errorReaction("Your memoized palindrome function returned the wrong result.");
                else
            ...
            if (speedup < 2.0)
                errorReaction("Should have been at least 2 times faster.\n (A correct memoized solution will "
                              "probably even be MUCH faster than that.)");
            else
                std::cout << " (Good!)" << std::endl;

            std::string found_palindrome = reconstructPalindrome(memo, str);
            std::cout << "Palindrome found (based on memoization): \"" << found_palindrome << "\""
                      << std::endl;
            if (found_palindrome != correct_palindrome_substring)
                errorReaction("The substring found is incorrect, so the memoization table\n is inaccurate or "
                              "incomplete.");
            else
                std::cout << " (Correct!)" << std::endl;
        }
