...
StringIntMap makeWordCounts(const StringVec& words)
{
    ...
    for (const std::string& str: words) {
        ...
        if (wordcount_map.find(key) == wordcount_map.end()) {
            // first time, key is not in map
            wordcount_map[key] = 1;
        } else {
            // not first time, key is in map
            wordcount_map[key] += 1;
        }
    }
...
int memoizedLongestPalindromeLength(LengthMemo& memo,
                                    ...
                                    double maxDuration)
{
    ...
    if (str.at(leftLimit) == str.at(rightLimit)) {
        ...
        int middleSubproblemResult =
            memoizedLongestPalindromeLength(memo, str, newLeft, newRight, startTime, maxDuration);
    ...
    int leftSubproblemResult =
        memoizedLongestPalindromeLength(memo, str, leftLimit, rightLimit - 1, startTime, maxDuration);
    // Move the left limit to the right and recurse.
    int rightSubproblemResult =
        memoizedLongestPalindromeLength(memo, str, leftLimit + 1, rightLimit, startTime, maxDuration);
