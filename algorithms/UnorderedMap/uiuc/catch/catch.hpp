...
// Specialised comparison functions to handle equality comparisons between ints and pointers (NULL deduces as
// an int)
template <typename LhsT, typename RhsT>
...
#define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                                             \
    do {                                                                                                   \
        Catch::AssertionHandler catchAssertionHandler(                                                     \
            macroName, CATCH_INTERNAL_LINEINFO, CATCH_INTERNAL_STRINGIFY(__VA_ARGS__), resultDisposition); \
        INTERNAL_CATCH_TRY                                                                                 \
        {                                                                                                  \
        ...
        INTERNAL_CATCH_REACT(catchAssertionHandler)                                                        \
             ...
             false && static_cast<bool>(!!(__VA_ARGS__))) // the expression here is never evaluated at runtime
                                                          // but it forces the compiler to give it a look
  // The double negation silences MSVC's C4800 warning, the static_cast forces short-circuit evaluation if the
  // type has overloaded &&.
...
public:
    ...
    void
    writeSection(std::string const& className, std::string const& rootName, SectionNode const& sectionNode);
...
void RunContext::assertionEnded(AssertionResult const& result)
{
    ...
    // We have no use for the return value (whether messages should be cleared), because messages were made
    // scoped and should be let to clear themselves out.
